...
#  config_path()
  ...
  fetchworker=BatchFetcher(datadir,True, False, True)
  fetchworker.start()
  fetchworker2=BatchFetcher(valdir,True, True, True)
  fetchworker2.start()
  ...
  #for epo in range(start_epoch,max_epoch+1):
    for i in range(start_step, 225*step_per_epoch):
      im_in, label_gt,names=fetchworker.fetch()
      net.set_feed(im_in, label_gt,i)
      net.run_optim(sess)
      ...
      if np.mod(global_step,10)==0:
        ...
        im_in,label_gt,names=fetchworker2.fetch()
        net.set_feed(im_in, label_gt,i)
        pred_class=net.run_result(sess)
...
#        except:
        # np.savez(os.path.join(sample_dir, '%s.npz'%(i)), im=im_in, gt_label=label_gt, names=names, pred_lay=pred_lay, pred_class=pred_class)
